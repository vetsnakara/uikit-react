...
function fonts() {
  return src(`${paths.from}/fonts/*.{woff,woff2}`).pipe(
    dest(`${paths.to}/fonts`)
  );
}

function legacyImages() {
  return src(`${paths.from}/images/*.{png,ico,jpg,svg}`).pipe(
    dest(`${paths.to}/images`)
  );
}
...
function autodoc(callback) {
  ...
  exec(execute, function (err, stdout, stderr) {
    callback(err);
  });
}

function autotest(callback) {
  exec("node autotest.js", function (err, stdout, stderr) {
    callback(err);
  });
}
